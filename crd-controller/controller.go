...
type Controller struct {
	lister   customlisters.EmptyAppLister
	queue    workqueue.RateLimitingInterface
	informer cache.Controller
}

func NewController(queue workqueue.RateLimitingInterface,
	lister customlisters.EmptyAppLister, informer cache.Controller) *Controller {
	return &Controller{
		informer: informer,
		lister:   lister,
		queue:    queue,
...
func (c *Controller) procesResource(key string) error {
	...
	app, err := c.lister.EmptyApps(namespace).Get(name)
	if err != nil {
		return err
	}
